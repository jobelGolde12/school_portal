        ...
        <div class="table table-striped table-responsive pb-5">
            ...
            <tbody class="t-body">
                <tr v-for="data in tableData" :key="data.index">
                    ...
                    <td>
                        <Link :href="route('editUserRoute', {id: data.id})" class="btn btn-primary me-3" :class="{'disabled' : data.type === 'superadmin'}"><i class="bi bi-pencil"></i></Link>
                        <button class="btn btn-warning" :class="{'disabled' : data.type === 'superadmin'}" data-bs-toggle="modal" data-bs-target="#exampleModal" @click="getIdToDelete(data.id)"><i class="bi bi-trash"></i></button>
                    </td>
                </tr>
            </tbody>
        </div>
